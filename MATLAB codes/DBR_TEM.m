function[]=DBR_TEM(n1,n2,N_sub,N_0,n_p,lbd,theta,dbr,mix_TEM)
...
while l_curr<loop
    ...
    if l_curr==0
        ...
        b_S0=(n0.*Cos_0)./(n_sub.*Cos_S);
        b_0S=(n_sub.*Cos_S)./(n0.*Cos_0);
    ...
    elseif l_curr==1
        ...
        b_0S=(n0.*Cos_S)./(n_sub.*Cos_0);
        b_S0=(n_sub.*Cos_0)./(n0.*Cos_S);
    ...
    P_S(1,1,:)=exp(-1i*kS*dsub);
    P_S(1,2,:)=zeros(size(l));
    P_S(2,1,:)=zeros(size(l));
    P_S(2,2,:)=exp(1i*kS*dsub);
    ...
    for ii= 1:il
        ...
        T_pair(:,:,ii)=T_0t(:,:,ii)*P_t(:,:,ii)*T_ts(:,:,ii)*P_s(:,:,ii)*T_st(:,:,ii)*P_t(:,:,ii)*((T_ts(:,:,ii)*P_s(:,:,ii)*T_st(:,:,ii)*P_t(:,:,ii))^(n_p-1))*T_tS(:,:,ii)*P_S(:,:,ii)*T_S0(:,:,ii);
    ...
    if l_curr==1
        ...
        R=R+(mix_TEM)*(abs(r_DBR).^2);
        
    else
        
        R=R+(1-mix_TEM)*(abs(r_DBR).^2);
    end
